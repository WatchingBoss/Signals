...
class Stock(Instrument):
    ...
    def save_candles(self, intervals: list) -> None:
        ...
        with pd.HDFStore(path, mode="w") as hdf:
            for interval in intervals:
                ...
                hdf.put(
                    key=f"{self.ticker}_{interval.value}",
                    value=df,
                    format="table",
                    data_columns=True,
                )
    ...
    def read_candles(self, intervals: list) -> None:
        ...
        with pd.HDFStore(path, mode="r") as hdf:
            for interval in intervals:
                self.timeframes[interval].df = hdf.get(
                    key=f"{self.ticker}_{interval.value}"
                )
        ...
        if not tf.df.size:
            ...
            if (datetime.now(timezone.utc) - tf.df["Time"].iat[-1]) > DELTAS[interval]:
                tf.df = pd.concat(
                    [
                        tf.df,
                        append_df(client, interval, tf.df["Time"].iat[-1], self.figi),
                    ],
                    ignore_index=True,
                )
    ...
    def fill_indicators(self, interval: Interval):
        ...
        tf.df[["MACD_12_26_9", "MACDh_12_26_9", "MACDs_12_26_9"]] = ta.macd(
            tf.df["Close"], fast=12, slow=26, signal=9
        )
    ...
    def fill_df_yahoo(self, interval):
        tf = self.timeframes[interval]
        tf.df = tf.df.ta.ticker(
            self.ticker, period="1mo", interval=YahooIntervals[interval]
        )


def append_df(
    client: ti.SyncClient, interval: Interval, last_time: datetime, figi: str
) -> pd.DataFrame:
    ...
    while oldest_time > last_time and break_loop < 100:
        try:
            ...
            print(
                f"Wating for 60 seconds -> {figi} -> {interval} -> {datetime.now().strftime('%H:%M:%S')}"
            )
            time.sleep(60)
...
def fill_df(client, interval, ticker, figi) -> pd.DataFrame:
    ...
    while break_loop < 100:
        ...
        try:
            candles = client.get_market_candles(
                figi, from_=now - PERIODS[interval], to=now, interval=interval
            ).payload.candles
            if len(candles) > 1:
            ...
            print(
                f"Wating for 60 seconds -> {ticker} -> {interval} -> {datetime.now().strftime('%H:%M:%S')}"
            )
            time.sleep(60)
